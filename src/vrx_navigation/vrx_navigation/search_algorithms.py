...
class DStarLitePlanner:
    def __init__(self, maze, start, goal):
        ...
        self.rhs = {}
        self.expansions = 0
        self.latest_expansions = 0 # NEW: Tracks nodes expanded during the most recent calculation
    ...
    def compute_shortest_path(self):
        while True:
            ...
            self.expansions += 1
            self.latest_expansions += 1 # NEW: Iterate local counter
    ...
    def update_map(self, new_maze, current_start):
        ...
        self.latest_expansions = 0 # NEW: Reset local counter before repair
        t_start = time.perf_counter()    
        self.compute_shortest_path()
        self.latest_compute_time = time.perf_counter() - t_start
        self.total_compute_time += self.latest_compute_time
    ...
    def extract_path(self, start_node):
        if self.get_g(start_node) == INF:
            return [], self.latest_expansions, self.expansions # NEW: Return both tracking values
        ...
        return path, self.latest_expansions, self.expansions
