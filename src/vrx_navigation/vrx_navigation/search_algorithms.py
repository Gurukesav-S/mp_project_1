...
def a_star(maze, start, goal, costs, heuristic):
    ...
    while not fringe.empty():
        ...
        if current_g > g_cost.get(current_node, INF):
            continue
            
        closed_set.add(current_node)
        
        # Swapped to the new 8-way neighbors from D* Lite
        neighbours = get_8way_neighbors(maze, current_node)
        
        for next_node in neighbours:
            # Swapped to use the exact 1.414 cost logic
            edge_cost = cost(maze, current_node, next_node)
            if edge_cost == INF:
                continue
